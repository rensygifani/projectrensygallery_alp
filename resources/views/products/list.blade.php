...
<div class="row g-3">
    ...
    <div class="col-6 col-md-3 col-lg-3 col-xl-3">
        <div class="pastel-card shadow-sm p-2">
            ...
            @if($product->image)
                <img src="{{ asset('storage/'.$product->image) }}" class="pastel-thumb-img mb-2" alt="{{ $product->name }}">
            @else
            ...
            <div class="text-center px-2">
                ...
                <div class="d-flex gap-1">
                    <a href="{{ route('products.show',$product->id) }}" class="btn btn-sm pastel-outline w-50">Show</a>
                    <a href="{{ route('products.edit',$product->id) }}" class="btn btn-sm pastel-warning w-50">Edit</a>
                    {{-- <a href="{{ route('cart.add', $product->id) }}" class="btn btn-sm pastel-btn w-100 mt-2">+ Add to Cart</a> --}}
                    <form action="{{ route('products.destroy', $product->id) }}" method="POST" class="w-33" onsubmit="return confirm('Are you sure you want to delete this product?');">
                        @csrf
                        @method('DELETE')
